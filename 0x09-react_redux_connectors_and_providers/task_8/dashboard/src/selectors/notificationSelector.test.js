...
describe("testing selectors", () => {
  ...
  it("test that getNotifications returns a list of the message entities within the reducer", () => {
    const getNotificationsSpy = jest.spyOn(selectors, "getNotificationsSelector");
  ...
  it("test that getUnreadNotifications return a list of the message entities within the reducer", () => {
    ...
    const getUnreadNotificationsSpy = jest.spyOn(selectors, "getUnreadNotificationsByType");
  ...
  it("test that unRead notifications and urgent Notifications are returned", () => {
    const action = setNotifications(data);
    newState = rootReducerSpy(state=initialRootState.initialNotifState, action);
    console.log("newState:", newState.toJS());
    newState = { notifications: newState };
    const getUnreadNotificationsSpy = jest.spyOn(selectors, "getUnreadNotificationsByType");
    const unReadAndUrgentMsgs = getUnreadNotificationsSpy(newState);
    console.log("filtered unread messages with urgent type", unReadAndUrgentMsgs.toJS());

    expect(getUnreadNotificationsSpy).toHaveBeenCalled();
    expect(getUnreadNotificationsSpy).toHaveBeenCalledWith(newState);
    expect(unReadAndUrgentMsgs.count()).toEqual(2);
    expect(unReadAndUrgentMsgs.valueSeq().every((item) => item.get("isRead") === false)).toBe(true);
    getUnreadNotificationsSpy.mockRestore();
  });
});
